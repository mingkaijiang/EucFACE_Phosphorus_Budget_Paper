...
make_fineroot_c_pool <- function(back.calculate,
    ...
    ### depth profile
    dDF <- read.csv("data/raw/FACE_PILOTROOT_RA_FINEROOTS_L1_20131201-20131231.csv")
    ...
    ### estimate 50-60 profile, assuming half
    ...
    reDF1 <- myDF[myDF$Depth == "0 - 10", c("Number", "Ring", "Date", "FRC", "FRB", "Depth")]
    ...
    reDF4 <- summaryBy(FRC+FRB~Number+Ring+Date, FUN=sum,
                       data=reDF2, na.rm=T, keep.names=T)
    
    reDF4$Depth <- "10_30"
    
    reDF5 <- summaryBy(FRC+FRB~Number+Ring+Date, FUN=sum,
    ...
    ### calculate total C
    totDF <- summaryBy(FRC+FRB~Number+Ring+Date, FUN=sum, 
                       data=depthDF, na.rm=T, keep.names=T)
    colnames(totDF) <- c("Number", "Ring", "Date", "FRC_0_60", "FRB_0_60")
    
    depthDF <- merge(depthDF, totDF, by=c("Number", "Ring", "Date"))
    
    depthDF$pct <- depthDF$FRC / depthDF$FRC_0_60
    depthDF$pct2 <- depthDF$FRB / depthDF$FRB_0_60
    
    sumDF <- summaryBy(pct+pct2~Ring+Depth, FUN=c(mean,sd),
    ...
    ###### second dataset
    # read in the csv
    frb1 <- read.csv("data/raw/EucFACERootsRingDateDepth.csv")
    ...
    write.csv(outDF, "data/processed/fineroot_c_pool.csv", row.names=F)
